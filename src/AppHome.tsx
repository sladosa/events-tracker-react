...
function ActivitiesView() {
  ...
  // categoryId = leaf category (group.category_id iz ActivityGroup)
  const handleDeleteActivity = async (sessionStart: string, categoryId: UUID): Promise<void> => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');

      // P2 arhitektura: brisanje sesije = leaf eventi + parent eventi te sesije.
      // Leaf eventi:   WHERE session_start=X AND category_id = leafId
      // Parent eventi: WHERE session_start=X AND chain_key   = leafId
      // Dva odvojena upita jer Supabase JS ne podržava OR filter direktno.
      ...
      toast.success(`Aktivnost obrisana (${eventIds.length} event${eventIds.length !== 1 ? 's' : ''})`);
