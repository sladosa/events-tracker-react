...
export interface CachedActivityData {
  ...
  attributesByCategory: Map<string, { id: UUID; name: string; data_type: string; unit: string | null }[]>;
}
...
async function _buildCategoryChain(leafCatId: UUID): Promise<{
  ...
  const { data: attrDefs } = await supabase
    ...
    .select('id, name, data_type, unit, category_id')
    .in('category_id', chainIds)
    .order('sort_order', { ascending: true }) as {
      data: { id: string; name: string; data_type: string; unit: string | null; category_id: string }[] | null;
    };

  const attributesByCategory = new Map<string, { id: UUID; name: string; data_type: string; unit: string | null }[]>();
  for (const attr of attrDefs ?? []) {
    const existing = attributesByCategory.get(attr.category_id) ?? [];
    existing.push({ id: attr.id as UUID, name: attr.name, data_type: attr.data_type, unit: attr.unit ?? null });
