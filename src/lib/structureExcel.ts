...
function writeAutomationsSheet(wb: ExcelJS.Workbook, nodes: StructureNode[]): void {
  ...
  const helpLines = [
    ...
    '  DateMap = "vrijednost=pravilo | ...". Pravila: same (= datum sesije),',
    '     next:N      = N-ti dan SLJEDEĆEG mjeseca',
    ...
    '  Izmjeri oba, ne pogađaj:',
    '     B = dan zadnje transakcije na izvodu',
    '     D = dan kad skupna naplata pogodi tekući račun',
    '  Pa odaberi:  D je u sljedećem mjesecu od B → next:D    (Mastercard: next:11)',
    '               D je u istom mjesecu kao B    → cutoff:B:D (Visa: cutoff:3:5)',
    '  Zašto Visa nije next:5: transakcija 4.6. pripada izvodu koji se zatvara 2.7.',
    '  i tereti se ~5.7.; next:5 bi je stavio na 5.6., dakle mjesec prerano.',
    '  A next:3 pogodi mjesec, ali upiše 3., kad novac još nije otišao.',
    '',
...
function writeListColumnsSheet(wb: ExcelJS.Workbook, nodes: StructureNode[]): void {
  ...
  const helpLines = [
    ...
    '         ⚠ trima se — rubni razmaci se gube. Koristi tijesan znak.',
