...
- **BUG-S103-IMPORT-GRANTEE:** ✅ RIJEŠEN (S103) — Import gumb (Activities Excel Import) nije bio skriven za read-only grantee-a (za razliku od Add Activity); read grantee je mogao otvoriti Import modal i doći do "Skip / Import as mine" ekrana iako ne smije pisati u bazu. Fix: `isReadOnlyGrantee` guard dodan na sva 3 mjesta gdje se Import gumb prikazuje (`ActivitiesTable.tsx` header + empty-state, `AppHome.tsx` mobile chip) — isti pattern kao Add Activity.
- **BACKLOG — "Import as mine" za write grantee unutar iste shared aree nema smisla:** Trenutni "Skip / Import as mine" dijalog (S44 smart import) dozvoljava write grantee-u da tuđe (ownerove) retke uveze kao duplikate pod svojim user_id ALI u ISTOJ shared kategoriji — što stvara besmislene duplikate bez ikakve koristi. Prava namjena za "preuzimanje tuđih podataka kao svojih" je: (1) `LeaveAreaModal` → "Detach with data" (već postoji, S73/S74) da se napusti shared area i podaci prebace u novu vlastitu strukturu, ILI (2) napustiti shared area pa normalan Excel Import (bez "Save as mine") u novu VLASTITU area — jer exportani xlsx već sadrži i Structure sheet i Events sheet, pa "Create categories & continue" flow (Korak 7, S32) sam kreira novu strukturu. Razmotriti: sakriti/onemogućiti "Import as mine" opciju kad je aktivan `sharedContext` (write grantee unutar tuđe aree), jer legitimni put za taj scenarij već postoji. Niska žurnost — flag samo, nije implementirano.
...
- **BUG-S103-ANYATTR:** "In any attribute" filter (`ATTR_FILTER_ANY` u `eventQueryBuilder.ts`) i dalje timeouta za grantee-e nakon `sql/031_rls_exists_fix.sql` (T-S103-1 ❌). Vjerojatan root cause (nije potvrđen EXPLAIN ANALYZE-om): `ILIKE` nije leakproof operator, pa Postgres ne smije reorganizirati uvjete — mora evaluirati `event_attr_select` RLS EXISTS (join events→categories→data_shares) za svaki redak `event_attributes` PRIJE filtriranja po `value_text ILIKE`, umjesto da prvo suzi skup. Za usporedbu: specifičan atribut (`.eq('attribute_definition_id', ...)`, leakproof) radi OK za grantee — Postgres smije prvo filtrirati pa tek onda RLS na malom skupu. Pravi fix vjerojatno traži SECURITY DEFINER RPC funkciju koja radi pretragu server-side izvan PostgREST `!inner` joina. Privremeno (S103, shippano): amber upozorenje u `AppHome.tsx` kad `sharedContext` postoji i `selectedFilterAttr === ATTR_FILTER_ANY` — opcija ostaje u dropdownu, ali korisnik je upozoren i upućen na Comment/specific-attribute filter. Radi OK za ownera (bez data_shares RLS overhead).
...
- ✅ `sql/031_rls_exists_fix.sql` pokrenut na TEST i PROD (odgođeno s 2026-07-01 na 2026-07-03 zbog Supabase platform incidenta — PROD bio Unhealthy) — `event_attr_select`/`event_attach_select` RLS: EXISTS umjesto IN(subquery); specific-attribute filter za grantee sad radi (T-S103-1 djelomično ✅, vidi BUG-S103-ANYATTR ispod)
...
- ❌→📋 **BUG-S103-ANYATTR** (otkriven tijekom T-S103-1 testiranja): "In any attribute" filter i dalje timeouta za grantee čak i nakon 031 fixa — `ILIKE` nije leakproof operator pa Postgres mora evaluirati RLS EXISTS za cijelu `event_attributes` tablicu prije filtriranja (vidi Open bugs sekciju za detalje). Privremeno: amber notice u `AppHome.tsx` kad je `sharedContext` aktivan + odabran "In any attribute". Pravi fix (SECURITY DEFINER RPC) odgođen.
- ✅ **BUG-S103-IMPORT-GRANTEE** (otkriven tijekom T-S103-2 testiranja, fiksano isti session): Import gumb (Activities Excel Import) nije bio skriven za read-only grantee-a; sad skriven na sva 3 mjesta (`ActivitiesTable.tsx` header + empty-state, `AppHome.tsx` mobile chip), isti pattern kao Add Activity guard
- 📋 Flag (NE implementirano): "Import as mine" za write grantee unutar postojeće shared aree stvara besmislene duplikate — pravi put je Leave Area (Detach with data) ili normalan re-import (bez "Save as mine") u novu vlastitu area nakon napuštanja; vidi Open bugs sekciju
- Export profili (`area.settings.export_profiles`) su area-level, ne per-user — svi korisnici s pristupom vide iste profile (bitno za T-S103-3 test setup)
- Svi T-S103-1..5 testovi izvršeni: T-S103-1 ❌ (bug ostaje, notice dodan), T-S103-2/3/4/5 ✅
- ✅ **T-S100-6/7 dovršeni** — `~` partial i `*` any-attribute kao Export Profile attrFilterRaw override potvrđeni (Filter sheet: `*: ~Parking`); Import Profile toast s column order+widths+filter overrides info potvrđen
- ✅ **Bugfix: `evaluateCommentTemplate` null detekcija** (`src/lib/commentTemplate.ts`) — otkriveno T-S95-10 testom na produkcijskom Financije template `{racun}/{tip}/{podtip}/{napomena}`: kad su SVI placeholderi prazni, doslovni `/` separatori između njih preživljavali su trim (`"///"`.trim() nije prazan string) → comment se spremao kao `"///"` umjesto `null`. Fix: broji koliko je placeholdera stvarno popunjeno (`filledCount`); ako template ima 1+ placeholder i nijedan nije popunjen → vrati `null` bez obzira na literal separatore. Template bez placeholdera (čisti literal tekst) i dalje se ponaša kao prije.

**Prioriteti za S104:**
1. **BUG-S102-DELETE fix** — live recount eventa prije isBlocked odluke u `StructureDeleteModal.tsx` (umjesto stale `node.eventCount`)
...
3. **Export + Python klasifikacija** — export obje area-e, Python skripta predlaže Tip/Podtip
4. **Bulk update** — reimport xlsx s ispravljenim Tip/Podtip vrijednostima
5. **Garmin/Sleep skripta** — kad se nađu DI-Connect-Wellness fajlove
