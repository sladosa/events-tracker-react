...
- **→ Rječnik `Izvod opis` → `Tip`/`Podtip` i „sličan opis nije duplikat"** preseljeni su u `data-prep_tools/CLAUDE.md` (S151; učita se sam uz alate).
**Mjerenje / usklađenje**
...
**→ E2E (Playwright) zamke** — preseljeno u `e2e/CLAUDE.md` (S151; učita se sam uz testove).
---
...
(→ alati iz `data-prep_tools/Financije/` su u `data-prep_tools/CLAUDE.md`, § Key files)
...
### → Nastavak (izvodi, rječnik, 1:N, Visa) — `data-prep_tools/CLAUDE.md` (S151)

⚠ Ondje su i pravila koja traži **app** ploha za izvode (autoritet iznosa, 1:N, rata po broju) — pri tom radu ga pročitaj ručno.
